...
# ----------------------------------------------------------------------------------------------------------------------
def create_lstm_v4_architecture(dataloader_, dict_metadata, D=16, verbose=True,):
...
class LSTM_v4(nn.Module):
    ...
    def __init__(self, 
        ...
        self.start_token = nn.Parameter(torch.randn(1, 1, self.D * bb_factor))
    ...
    def forward(self, *args):
        ...
        decoder_input = self.start_token.to(enc_out.device).repeat(B, self.W_out, 1)  # (B, W_out, D)

        # concatenate exogenous features if present
        if exo_seq is not None:
            decoder_input = torch.cat([decoder_input, exo_seq], dim=2)
