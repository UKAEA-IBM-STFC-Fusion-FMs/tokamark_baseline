...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="/config_cnn_batch_64.yaml",
    ...
    parser.add_argument(
        ...
        # default="/config_cnn_test.yaml",
        default=1,
        help="Run number of the trained model",
    )
    ...
    cnn_model = create_cnn_v2_architecture(
        test_dataloader_eval, **config_cnn["cnn_settings"], verbose=True
    )
    ...
    model_dir = base_model_dir.rstrip("/") + f"/run_{args.run}/"

    # -------------------------------------------------------------------
    # Evaluation loop
    # -------------------------------------------------------------------

    # cnn_sanity_vizu_per_shot(test_dataloader_vizu, config_task, cnn_model, model_dir)
    # cnn_unstd_evaluation_per_shot(test_dataloader_eval, config_task, cnn_model, model_dir)
    cnn_unstd_evaluation_per_shot(test_dataloader_vizu, config_task, cnn_model, model_dir)
